...
{ 
  ...
  R = length(unique(pop.yr$age)) # number of age categories
  ...
  colnames(prob.matrix)<-seq(min(pop.yr$age),max(pop.yr$age),1)
  ...
  af_model <- af_model %>% 
              mutate(age=as.factor(seq(min(pop.yr$age),max(pop.yr$age),1))) %>%
              relocate(age)
  ...
  if(sample_type=="FOS"){
    ...
    alk.noage <- alk
    alk.aged <- alk
    pop.yr.noage <- pop.yr[!(pop.yr$id %in% alk$id),]
    ...
    colnames(prob.matrix.sub)<-seq(min(pop.yr$age),max(pop.yr$age),1)
    ...
    alk_mla_model <- alk_in_mla_model %>%
      group_by(age) %>%
      summarize(mean_SL_model=mean(standardLength_mm),
                sd_SL_model=sd(standardLength_mm))
  ...
  if(sample_type=="Hybrid"){
    ...
    basic_POS_samps <- mutate(basic_POS_props, binL_num = as.numeric(as.character(bin)))
    basic_POS_samps$hybrid_n <- ifelse(basic_POS_samps$prop<=bin_n,basic_POS_samps$prop,round((basic_POS_samps$prop*percent),0))
